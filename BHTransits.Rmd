...
sseq <- aseq %>% 
  ...
  mutate(prev_sid = lag(sid),
         ...
         BSDirect = case_when(
           ...
           !(first(sloc) %in% c("Bathampton_W","Swainswick_S")) | !(last(sloc) %in% c("Bathampton_E","Swainswick_N")) ~ "Not A36-A46",
...
```{r age and type mix}
car_van_age <- sseq_t %>% 
  filter(type %in% c("CARS", "LCVs"),
         BSDirect != "Not A36-A46") %>% 
  ungroup() %>% 
  select(vehicle_id, visit, BSDirect, time) %>% 
  group_by(vehicle_id, visit) %>% 
  #time will be entry time for visit
  #BSDirect should be 'Direct', or 'London' if any are London, else 'Other'
  summarise(time = min(time),
            BSDirect = min(BSDirect)) %>% 
  left_join(veh, by = c("vehicle_id" = "id"))  %>% 
  mutate(EISY = as.integer(substr(intro_date, 1, 4)),
         tbin = lubridate::round_date(time, "1 hour"),
         ebin = if_else(
           euro_status %in% c("E4", "E5", "E6"), euro_status, "Other"),
         cbin = if_else((fuel_type=="PETROL" & euro_status %in% c("E4","E5","E6"))|(fuel_type=="DIESEL" & euro_status == "E6"),
                        "Low", "High/Unknown"))
...
ggplot(car_van_age, aes(x = tbin)) + geom_bar() +
  facet_grid(cbin ~ BSDirect) + 
  ggtitle("Cars & LCVs transiting Bath (A36-A46)", "By CAZ Emissions Level (Low = OK), Route and hour of day. 2-day sample.") +
  labs(x = "Hour of Day", y= "Number of Vehicle-Visits")
